...
class NCEData(object):
    ...
    def __init__(self, dataset, batch_size, context_size,
                 num_noise_words, max_size, num_workers, vocab_='content'):
...
class _NCEGenerator(object):
    ...
    def next(self):
        ...
        while len(batch) < self.batch_size:
            if prev_doc_id == len(self.dataset):
                ...
                batch.torch_()
                return batch
            if prev_in_doc_pos <= (len(self.dataset[prev_doc_id].content) - 1
                                   - self.context_size):
    ...
    def _num_examples_in_doc(self, doc, in_doc_pos=None):
        if in_doc_pos is not None:
            # number of remaining
            if len(doc.content) - in_doc_pos >= self.context_size + 1:
                return len(doc.content) - in_doc_pos - self.context_size
            return 0

        if len(doc.content) >= 2 * self.context_size + 1:
            # total number
            return len(doc.content) - 2 * self.context_size
        return 0

    def _add_example_to_batch(self, doc_id, in_doc_pos, batch):
        doc = self.dataset[doc_id].content
...
class _NCEGeneratorState(object):
    ...
    def _advance_indices(self, dataset, batch_size,
        ...
        self._in_doc_pos.value = (len(dataset[self._doc_id.value].content)
                                  - context_size
